...
public class ExecuteTester {
    ...
    /**
     * For testing the execute() methods from your parser without requiring the
     * world or the game:
    ...
    private static final String[][][] programs = new String[][][] {
            { // STAGE 0
                    ...
                    { "takeFuel;", "takeFuel" },
                    { "wait;", "wait" },
                    { "wait; loop { move; turnL;}",
                            "wait move turnL move turnL move turnL move turnL move turnL move turnL move turnL move turnL move turnL move" },
                    { "wait; loop { turnR; loop { move; } turnL;}",
                            "wait turnR move move move move move move move move move move move move move move move move move move" }
            },
            { // STAGE 1:
                    { "shieldOn;", "shieldOn" },
                    { "shieldOff;", "shieldOff" },
                    ...
                    { "wait; while (lt(wallDist, 3)) { turnL; } move;",
                            "wait getWallDist turnL getWallDist turnL getWallDist turnL getWallDist move" },
                    { "wait; while (gt(fuelLeft, 1)) { turnL; } move;",
                            "wait getFuelLeft turnL getFuelLeft turnL getFuelLeft turnL getFuelLeft turnL getFuelLeft move" }
            },
            { // STAGE 2
            ...
            { // STAGE 3
              // for the variable assignments, put spaces around =
                    ...
                    { "$a = 3; move($a);turnL;$a = 5; move($a);turnR;",
                            "move move move turnL move move move move move turnR" },
                    { "$a = 3 ; $b = sub($a, 1); move($b); turnL;", "move move turnL" },
                    { "$a = 3; $abcd = 2; move($abcd);turnL;", "move move turnL" },
                    { "$b = mul(2,fuelLeft); move($b); turnL;",
                            "getFuelLeft move move move move move move move move move move turnL" },
                    { "$a = 2; while(lt($a, fuelLeft)){$a = add($a,1); move;} turnL;",
                            "getFuelLeft move getFuelLeft move getFuelLeft turnL" },
                    { "$a = 3; $b = 4; if(eq($a, mul($b,3))){if (lt(0,barrelLR(mul($a, $b)))) {move($b); turnL;}} else {turnR;}",
                            "turnR" },
                    { "$a = 12; $b = 4; if(eq($a, mul($b,3))){if (lt(0,barrelLR(mul($a, $b)))) {move($b); turnL;}} else {turnR;}",
                            "getBarrelLR(48) move move move move turnL" }
            }
    };
 ...
 * OP ::= "add" | "sub" | "mul" | "div"
 * COND ::= RELOP "(" EXPR "," EXPR ") | "and" "(" COND "," COND ")" | "or" "("
 * COND "," COND ")" | "not" "(" COND ")"
 * 
 * Stage 3 additions
 * 
